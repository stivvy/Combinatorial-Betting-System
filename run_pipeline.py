...
"""
================================================================
 JACKPOT PREDICTION SYSTEM — MASTER RUNNER  v6.0
 Phase 1 (odds model) -> Phase 2 (wheel) -> Phase 3 (filter)

 Usage:
   python run_pipeline.py --matches matches.csv --budget 500
   python run_pipeline.py --matches matches.csv --budget 300 --threshold 0.50
   python run_pipeline.py --matches matches.csv --budget 500 --relax
================================================================
"""
...
def main():
    ...
    if not os.path.exists(args.matches):
        print(f"{C.RE}  File not found: {args.matches}{C.R}"); sys.exit(1)
    ...
    print(f"""
{C.YE}{C.B}  ╔══════════════════════════════════════════════════════════╗
  ║      JACKPOT PREDICTION SYSTEM  v6.0 (Odds-Based)       ║
  ║   Odds Model -> Wheel -> Filter                         ║
  ╚══════════════════════════════════════════════════════════╝{C.R}
  ...
  Filter:     {mode}
  Data:       Bookmaker odds (no API needed){C.R}
""")

    # Phase 1
    p1 = [script("phase1_model.py"), "--matches", args.matches,
          "--threshold", str(args.threshold)]
    run(p1, "Phase 1: Odds Model")

    # Phase 2
    p2 = [script("phase2_wheel.py"), "--budget", str(args.budget),
